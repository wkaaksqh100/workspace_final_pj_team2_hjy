...
const BuySelectTbl = () => {
    ...
    const [buyOrderList, setBuyOrderList] = useState([]); // 초기값을 모르므로 빈배열로 buyList에 대입

    // fecth()를 통해 톰캣서버에세 데이터를 요청
    useEffect(() => {
        fetch("http://localhost:8081/main/buyOrderList", {
            method: "GET"
        })
            ...
            .then(res => {
                console.log(1, res);
                setBuyOrderList(res || []); // 처음에는 비어있으므로 못가져온다. setBoardList(res);
            }
            )
            .catch(error => {
                console.error("데이터 가져오기 오류:", error);
                setBuyOrderList([]); // 오류 발생 시 빈 배열 설정 
            });
    ...
    const deleteOrderItem = (order_id) => {
        ...
        fetch("http://localhost:8081/main/orderItem/" + order_id, {
            method: 'DELETE',
        })
            .then((res) => res.text())
            .then((res) => {
                if (res === "ok") {
                    alert('삭제 성공!');
                    setBuyOrderList(buyOrderList.filter(order => order.order_id !== order_id)); // UI 업데이트
                } else {
                    alert('삭제 실패');
                }
            })
    ...
    return (
        <>
            <Table height={500} data={buyOrderList} style={{ maxWidth: 1500 }}>
