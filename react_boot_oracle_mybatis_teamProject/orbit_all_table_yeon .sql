...
VALUES (1, '2025-04-02', 502, 3, 3, '부가세율 적용', '2025-04-12', null, 1, 2);
...
create table order_item_tbl_hjy(
	...
	CONSTRAINT fk_quantity_tbl FOREIGN KEY (order_id) REFERENCES order_tbl_hjy (order_id) ON DELETE cascade ON update CASCADE,
    CONSTRAINT fk_item_info_tbl FOREIGN KEY (item_code) REFERENCES item_info_tbl_hjy (item_code) ON DELETE cascade ON update CASCADE
) ENGINE=InnoDB;

SELECT * FROM order_item_tbl_hjy;

insert into order_item_tbl_hjy 
values(1, 1011, '에어팟3' ,30);
--------------------------------------------------------------------------------------
-- 물품 금액 테이블 생성
...
create table item_price_history_tbl​_hjy(
	...
	vat DECIMAL(12,2),
	total DECIMAL(12,2),
    constraint fk_item_info FOREIGN KEY (item_code) REFERENCES item_info_tbl_hjy (item_code) ON DELETE cascade ON update CASCADE,
    constraint fk_order_tbl_price FOREIGN KEY (order_id) REFERENCES order_tbl_hjy (order_id) ON DELETE cascade ON update CASCADE
)ENGINE=InnoDB;
